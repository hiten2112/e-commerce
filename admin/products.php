  ...
  if($_POST) {
   ...
   $errors= array();
    ...
    if (!empty($_FILES)) {
      ...
      $dbpath = '/tutorials/images/products/'.$uploadName;
      if($mimeType != 'image'){
  ...
  <tbody>
   <?php while($product = mysqli_fetch_assoc($presults)):
     ...
     <tr>
       ...
       <td><?=$category;?></td>
       <td><a href="products.php?featured=<?=(($product['feature'] == 1)?'1':'0');?>&id=<?=$product['id'];?>" class="btn btn-xs btn-default ">
         <span class="glyphicon glyphicon-<?=(($product['feature'] ==0)?'minus':'plus');?>"> </span>
       </a>&nbsp <?=(($product['feature'] == 1)?'Featured products':'');?></td>
       <td>0</td>
     </tr>
   <?php endwhile;?>
  </tbody>
